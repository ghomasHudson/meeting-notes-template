 ...
 */
function get_rss_feed_as_html($feed_url, $max_item_cnt = 0, $show_date = true, $show_description = true, $max_words = 0, $cache_timeout = 0, $cache_prefix = "/tmp/rss2html-")
{
    $result = "";
    ...
    $result .= '<table style="border:solid; border-color:#d4d0c8;width:100%"><tbody>';
    for ($x=0;$x<$max_item_cnt;$x++) {
        $title = str_replace(' & ', ' &amp; ', $feed[$x]['title']);
        $link = $feed[$x]['link'];
	$date = date('l F d, Y', strtotime($feed[$x]['date']));
	$result .= '<tr><td style="background-color:#BBBBBB"><strong><a style="color:black" href="'.$link.'" title="'.$title.'">'.$date.'</a></strong></td></tr>';
	$result .= '<tr><td>';
        if ($show_description) {
            ...
            // no html tags
            $description = preg_replace('/(<(script|style)\b[^>]*>).*?(<\/\2>)/s', "$1$3", $description);
